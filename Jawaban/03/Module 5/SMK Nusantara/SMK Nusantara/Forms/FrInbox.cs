...
{
    public partial class FrInbox : Form
    {
        public FrInbox(User user)
        {
            InitializeComponent();
            this.user = user;
        }
        ...
        private void RefreshDGVMesage()
        {
            using (SampleDataContext db = new SampleDataContext())
            {
                DGVMessage.DataSource = from m in db.Messages
                                        ...
                                        where m.RecipientId == user.userid
                                        select new
                                        {
                                            m.MessageId,
                                            From = u.username,
        ...
        private void SearchByTitle()
        {
            using (SampleDataContext db = new SampleDataContext())
            {
                    ...
                    DGVMessage.DataSource = from m in db.Messages
                                            join u in db.Users
                                            on m.AuthorId equals u.userid
                                            where m.RecipientId == user.userid && SqlMethods.Like(m.Title, "%"+ TbxSearch.Text +"%")
                                            select new
                                            {
                                                m.MessageId,
                                                From = u.username,
        ...
        private void SearchByNameStudent()
        {
            using (SampleDataContext db = new SampleDataContext())
            {
                ...
                DGVMessage.DataSource = from m in db.Messages
                                        ...
                                        where m.RecipientId == user.userid && SqlMethods.Like(st.Name, "%" + TbxSearch.Text + "%")
                                        select new
                                        {
                                            m.MessageId,
                                            From = u.username,
        ...
        private void SearchByNameTeacher()
        {
            using (SampleDataContext db = new SampleDataContext())
            {
                ...
                DGVMessage.DataSource = from m in db.Messages
                                        ...
                                        where m.RecipientId == user.userid && SqlMethods.Like(t.Name, "%" + TbxSearch.Text + "%")
                                        select new
                                        {
                                            m.MessageId,
                                            From = u.username,
        ...
        private void SearchByContent()
        {
            using (SampleDataContext db = new SampleDataContext())
            {
                ...
                DGVMessage.DataSource = from m in db.Messages
                                        join u in db.Users
                                        on m.AuthorId equals u.userid
                                        where m.RecipientId == user.userid && SqlMethods.Like(m.Content,"%" + TbxSearch.Text +"%")
                                        select new
                                        {
                                            m.MessageId,
                                            From = u.username,
